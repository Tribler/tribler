...
class LengthSelectionStrategy(SelectionStrategy):
    ...
    def can_select(self, circuits_to_select_from):
        logger.error("Trying to select from {} with length between {} and {}".format({c.circuit_id: len(c.hops) for c in circuits_to_select_from}, self.min, self.max))
    ...
    def select(self, circuits_to_select_from):
        if not self.can_select(circuits_to_select_from):
            logger.error("ERROR select from {} with length between {} and {}".format({c.circuit_id: len(c.hops) for c in circuits_to_select_from}, self.min, self.max))
