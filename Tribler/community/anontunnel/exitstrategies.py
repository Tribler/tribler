...
class DefaultExitStrategy(TunnelObserver):
    ...
    def on_exiting_from_tunnel(self, circuit_id, return_candidate, destination,
        ...
        try:
            self.get_exit_handler(circuit_id, return_candidate).sendto(data,
                                                                       destination)
        except socket.error:
    ...
    def create(proxy, raw_server, circuit_id, address):
        ...
        # on_data event.
        if circuit_id in proxy.circuits and proxy.circuits[
            circuit_id].goal_hops == 0:
            return_handler = exitsocket.ShortCircuitExitSocket(raw_server,
                                                               proxy,
                                                               circuit_id,
                                                               address)
        else:
