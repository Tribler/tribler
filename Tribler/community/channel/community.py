...
class ChannelCommunity(Community):
        ...
        for message in messages:
            ...
            if message.payload.modification_on.name == u"torrent" and message.payload.modification_type == u"metadata-json":
                try:
                    ...
                    torrent_id = self._channelcast_db._sqlite_cache_dbfetchone(
                        u"SELECT torrent_id FROM _ChannelTorrents WHERE dispersy_id = ?",
                        (modifying_dispersy_id,))
                    infohash = self._channelcast_db._sqlite_cache_dbfetchone(
    ...
    def _get_playlist_id_from_message(self, dispersy_id):
        ...
        return self._channelcast_db._sqlite_cache_dbfetchone(u"SELECT id FROM _Playlists WHERE dispersy_id = ?", (dispersy_id,))
    ...
    def _get_torrent_id_from_message(self, dispersy_id):
        ...
        return self._channelcast_db._sqlite_cache_dbfetchone(u"SELECT id FROM _ChannelTorrents WHERE dispersy_id = ?", (dispersy_id,))
    ...
    def _get_latest_modification_from_channel_id(self, type_name):
        ...
        # 1. get the dispersy identifier from the channel_id
        dispersy_ids = self._channelcast_db._sqlite_cache_dbfetchall(
    ...
    def _get_latest_modification_from_torrent_id(self, channeltorrent_id, type_name):
        ...
        # 1. get the dispersy identifier from the channel_id
        dispersy_ids = self._channelcast_db._sqlite_cache_dbfetchall(u"SELECT dispersy_id, prev_global_time " + \
