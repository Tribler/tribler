...
class StoreRequestPayload(BasePayload):
    ...
    format_list = BasePayload.format_list + ['20s', 'varlenH']

    def __init__(self, identifier, target, values):
    ...
    @classmethod
    def from_unpack_list(cls, identifier, target, values_str):
        values = decode_values(values_str)
        return StoreRequestPayload(identifier, target, values)
...
class FindResponsePayload(BasePayload):
    ...
    format_list = BasePayload.format_list + ['varlenH', 'varlenH']

    def __init__(self, identifier, values, nodes):
    ...
    @classmethod
    def from_unpack_list(cls, identifier, values_str, nodes_str):
        values = decode_values(values_str)
