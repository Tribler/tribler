...
class TestTorrentMD(TestAsServer):
    ...
    def test_create_torrent_md(self):
        with db_session:
            self.session.mds.TorrentMD()
            self.assertEqual(
                orm.select(g for g in self.session.mds.TorrentMD).count(),
                1)
    ...
    def test_create_from_tdef(self):
        with db_session:
            ...
            md = self.session.mds.TorrentMD.from_tdef(key, tdef)
            orig = {
    ...
    def test_search_keyword(self):
        with db_session:
            ...
            self.assertEqual(
                md1.rowid,
                self.session.mds.TorrentMD.search_keyword("foo")[0].rowid)
            self.assertEqual(
                md2.rowid,
                self.session.mds.TorrentMD.search_keyword("eee")[0].rowid)
            self.assertEqual(
                2, len(
                    self.session.mds.TorrentMD.search_keyword("123")))
            self.assertEqual(
                3, len(
    ...
    def test_search_keyword_stemmed(self):
        with db_session:
            ...
            self.assertEqual(
                md.rowid,
                self.session.mds.TorrentMD.search_keyword("mountain")[0].rowid)
            self.assertEqual(
                md.rowid,
                self.session.mds.TorrentMD.search_keyword("sheeps")[0].rowid)
    ...
    def test_from_dict(self):
        with db_session:
            ...
            chan = self.session.mds.TorrentMD.from_dict(key1, d)
