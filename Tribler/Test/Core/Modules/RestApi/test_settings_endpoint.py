...
class TestSettingsEndpoint(AbstractApiTest):
    ...
    def test_unicode_chars(self):
        ...
        post_data_dict = {'watch_folder': {'watch_folder_dir': "".join([unichr(i) for i in range(256)])}}
        post_data = json.dumps(post_data_dict, ensure_ascii=False)

        def check_settings(settings):
            watch_folder = json.loads(settings)['settings']['watch_folder']['watch_folder_dir']
            self.assertEqual(watch_folder, post_data_dict['watch_folder']['watch_folder_dir'])
    ...
    def test_set_settings(self):
        ...
        post_data = json.dumps({'general': {'family_filter': not old_filter_setting},
                                ...
                                'downloadconfig': {'seeding_mode': 'time', 'seeding_time': 100}})
        ...
        post_data = json.dumps({'downloadconfig': {'seeding_mode': 'ratio', 'seeding_ratio': 3}})
        yield self.do_request('settings', expected_code=200, request_type='POST', post_data=post_data, raw_data=True) \
