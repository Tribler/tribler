...
class TestKatPhTorrentFinderPlugin(unittest.TestCase):
    ...
    def test_ImportPlugin(self):
        ...
        #Assert
        assert len(plugins) <= 1, "Found more ITorrentFinderPlugins than just the KatPhTorrentFinderPlugin"
        assert len(plugins) == 1, "Did not find the KatPhTorrentFinderPlugin"
    ...
    def test_LoadTorrentList(self):
        ...
        pluginmanager.RegisterCategory("TorrentFinder", ITorrentFinderPlugin)
        pluginmanager.LoadPlugins()
        plugin = pluginmanager.GetPluginsForCategory("TorrentFinder")[0]
        #Load a torrent
