...
class MockAllChannelCommunity(AllChannelCommunity):
    def __init__(self, *args, **kwargs):
        super(MockAllChannelCommunity, self).__init__(*args, **kwargs)

        self.callbacks = {}

    def add_callback(self, name, func, stop):
        self.callbacks[name] = (func, stop)

    def on_trap_function(self, name, *args):
        if name in self.callbacks:
            f, stop = self.callbacks[name]
            f(*args)
            return stop
        return False

    def on_channelcast(self, messages):
        if self.on_trap_function("on_channelcast", messages):
            return
        super(MockAllChannelCommunity, self).on_channelcast(messages)

    def on_channelcast_request(self, messages):
        if self.on_trap_function("on_channelcast_request", messages):
            return
        super(MockAllChannelCommunity, self).on_channelcast_request(messages)

    def on_channelsearch(self, messages):
        if self.on_trap_function("on_channelsearch", messages):
            return
        super(MockAllChannelCommunity, self).on_channelsearch(messages)

    def on_channelsearch_response(self, messages):
        if self.on_trap_function("on_channelsearch_response", messages):
            return
        super(MockAllChannelCommunity, self).on_channelsearch_response(messages)

    def on_votecast(self, messages):
        if self.on_trap_function("on_votecast", messages):
            return
        super(MockAllChannelCommunity, self).on_votecast(messages)

    def undo_votecast(self, messages):
        if self.on_trap_function("undo_votecast", messages):
            return
        super(MockAllChannelCommunity, self).undo_votecast(messages)
...
class AllChannelTestBase(AbstractServer):
    ...
    def setUp(self):
        ...
        self.community1 = self.dispersy1.define_auto_load(
            MockAllChannelCommunity, self.node1, load=True, kargs={'tribler_session': self.ses1})[0]
        ...
        self.community2 = self.dispersy2.define_auto_load(
            MockAllChannelCommunity, self.node2, load=True, kargs={'tribler_session': self.ses2})[0]
        # Communicate nodes
