...
from Tribler.vwxGUI.libraryItem import libraryItem
...
class StaticGridPanel(wx.Panel):
    ...
    def calculateRows(self, event=None):
        ...
        if size[1] < 50 or self.subPanelHeight == 0:
            ...
            print >> sys.stderr, 'libraryGrid: Height: %d, single panel is %d, so %d rows' % (size[1], self.subPanelHeight, self.currentRows)
