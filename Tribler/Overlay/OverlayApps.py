...
from BitTornado.BT1.MessageID import HelpCoordinatorMessages, HelpHelperMessages, MetadataMessages, BuddyCastMessages, DIALBACK_REQUEST, getMessageName
...
class OverlayApps:
    ...
    def register(self, secure_overlay, launchmany, rawserver, config):
        ...
        if config['buddycast']:
            ...
            self.buddycast.register(secure_overlay, launchmany.rawserver, launchmany, launchmany.listen_port, launchmany.exchandler)
        ...
        if config['torrent_collecting'] or config['download_help']:
            # Create handler for metadata messages
            self.metadata_handler = MetadataHandler.getInstance()
            self.metadata_handler.register(secure_overlay, self.help_handler, launchmany, config['config_path'], config['max_torrents'])
        ...
        if config['dialback']:
            ...
            self.dialback_handler.register(secure_overlay,launchmany.rawserver,launchmany,launchmany.handler,launchmany.listen_port,config['max_message_length'],config['dialback_active'],config['dialback_trust_superpeers'],config['dialback_interval'])
            self.register_msg_handler([DIALBACK_REQUEST],self.dialback_handler.handleSecOverlayMessage)
    ...
    def handleConnection(self,exc,permid,selversion,locally_initiated):
        ...
        if exc is None:
            ...
            dmh.handleSecOverlayConnection(permid,selversion,locally_initiated)
