...
class SelectedChannelList(GenericSearchList):
        ...
        if len(playlists) > 0 or len(torrents) > 0:
            ...
            if shouldDrag:
                data += [(
                         ...
                         [torrent.name,
                          torrent.length,
                          self.category_names[torrent.category_id],
                             torrent.num_seeders,
                             torrent.num_leechers,
                             0,
                             None],
                         torrent,
                         DragItem) for torrent in torrents]
                ...
                data += [(
                         ...
                         [torrent.name,
                          ...
                          self.category_names[torrent.category_id],
                             ...
                             None],
                         torrent,
                         ThumbnailListItem) for torrent in torrents]
            else:
                data += [(
                         torrent.infohash,
                         [torrent.name,
                          ...
                          self.category_names[torrent.category_id],
                             ...
                             None],
                         torrent,
                         TorrentListItem) for torrent in torrents]
            ...
            if isinstance(data, Torrent):
                if self.state == ChannelCommunity.CHANNEL_OPEN or self.iamModerator:
                    data = (
                        ...
                        [data.name,
                         data.length,
                         self.category_names[data.category_id],
                         data.num_seeders,
                         data.num_leechers,
                         0,
                         None],
    ...
    def _ShowFavoriteDialog(self, nrdownloaded):
        dial = wx.MessageDialog(
            None,
            "You downloaded %d torrents from this Channel. 'Mark as favorite' will ensure that you will always have access to newest channel content.\n\nDo you want to mark this channel as one of your favorites now?" %
            nrdownloaded,
            'Mark as Favorite?',
            wx.YES_NO | wx.YES_DEFAULT | wx.ICON_QUESTION)
...
class ManageChannelPlaylistsManager(BaseManager):
    ...
    def GetTorrentsFromChannel(self):
        delayedResult = startWorker(
            None,
            self.channelsearch_manager.getTorrentsFromChannel,
            wargs=(self.channel,
                   ),
            wkwargs={'filterTorrents': False},
            retryOnBusy=True,
            priority=GUI_PRI_DISPERSY)
        total_items, nrfiltered, torrentList = delayedResult.get()
        return torrentList
    ...
    def GetTorrentsNotInPlaylist(self):
        delayedResult = startWorker(
            None,
            self.channelsearch_manager.getTorrentsNotInPlaylist,
            wargs=(self.channel,
                   ),
            wkwargs={'filterTorrents': False},
            retryOnBusy=True,
    ...
    def GetTorrentsFromPlaylist(self, playlist):
        delayedResult = startWorker(
            None,
            self.channelsearch_manager.getTorrentsFromPlaylist,
            wargs=(playlist,
                   ),
            wkwargs={'filterTorrents': False},
            retryOnBusy=True,
    ...
    def createPlaylist(self, name, description, infohashes):
        startWorker(
            None,
            self.channelsearch_manager.createPlaylist,
    ...
    def savePlaylist(self, playlist_id, name, description):
        startWorker(
            None,
            self.channelsearch_manager.modifyPlaylist,
            wargs=(self.channel.id,
                   playlist_id,
                   name,
                   description),
    ...
    def savePlaylistTorrents(self, playlist_id, infohashes):
        startWorker(
            None,
            self.channelsearch_manager.savePlaylistTorrents,
            wargs=(self.channel.id,
                   playlist_id,
                   infohashes),
            retryOnBusy=True,
