...
class ChResultsItemPanel(wx.Panel):
    ...
    def getColumns(self):
        return [{'sort':'name', 'reverse':True, 'title':'Channel Results', 'width':242,'tip':self.utility.lang.get('C_filename'), 'order':'down'}
                ]     
    ...
    def setData(self, data):
        ...
        if data == "mychannel":
            ...
            votecast_db = VoteCastDBHandler.getInstance()
            ...
            ##print >> sys.stderr , "torrentList : " , self.torrentList

            ##self.torrentList = self.torrent_db.valuelist2torrentlist(self.torrent_db.value_name, self.torrentList)
            ...
            ##print >> sys.stderr , "TorrentList again" , self.torrentList            
    ...
    def setSubscribed(self):
        votecast_db = VoteCastDBHandler.getInstance()
        if votecast_db.hasVote(self.publisher_id, self.utility.session.get_permid()):
    ...
    def SubscriptionClicked(self, event):
        if self.SubscriptionButton.isToggled():
            ...
            votecast_db = VoteCastDBHandler.getInstance()
            votecast_db.deleteVote(self.publisher_id ,self.utility.session.get_permid())
    ...
    def mouseAction(self, event):
        ...
        if event.LeftUp():
            ...
            self.select()
            self.channelsDetails.loadChannel(self, self.torrentList, self.publisher_id, self.publisher_name, self.subscribed)
