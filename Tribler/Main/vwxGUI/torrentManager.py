...
from Tribler.Core.Search.KeywordSearch import KeywordSearch ## Tribler.Search
try:
    import web2
except ImportError:
...
#        self.loadData()
    ...
    def getCategory(self, categorykey, mode):
        ...
        def torrentFilter(torrent):
            ...
            if categorykey == 'all':
                if library:
                    okCategory = True # Also show torrents of disabled cats in library
                else:
        ...
        if mode == 'filesMode' and web2on and self.inSearchMode(mode) and \
                ...
                q = " ".join(self.searchkeywords[mode])
                stopWebsearch = self.category.family_filter_enabled() and self.category.xxx_filter.isXXX(q)
                if stopWebsearch:
                    return data
                
                self.dod = web2.DataOnDemandWeb2(q)
...
#                self.standardOverview.setSearchFeedback('torrent', True, len(data))                
    ...
    def getTorrents(self, hash_list):
        """builds a list with torrents that have the infohash in the list provided as an input parameter
            NB: Torrents in inactive categories are not returned!!!
        """
        torrent_list = []
                
        for hash in hash_list:
            torrent = self.info_dict.get(hash)
            if torrent and self.category.hasActiveCategory(torrent):
                torrent_list.append(torrent)
            else:
                # Also try to find the torrent in the remote search results
                # print 'Searching for: %s' % `hash`
                #remoteHashes = [repr(torrent['infohash']) for torrent in self.remoteHits[1]]
                #remoteHashes.sort()
                #for rhash in remoteHashes:
                #    print rhash
                remoteTorrent  = [ torrent for torrent in self.remoteHits[1] if torrent['infohash'] == hash]
                if remoteTorrent and self.category.hasActiveCategory(remoteTorrent):
                    torrent_list.append(remoteTorrent[0])
        return torrent_list
        ...
        if self.remoteHits and self.remoteHits[0] == self.searchkeywords[mode]:
            ...
            def catFilter(item):
                ...
                if type(icat) == list:
                    ...
                    return False
                return icat == cat or (cat == 'all' and icat in [a[0].lower() for a in self.category.getCategoryNames()])
    ...
    def gotRemoteHits(self,permid,kws,answers,mode='filesMode'):
        ...
        try:
            ...
            if self.searchkeywords[mode] == kws and self.standardOverview.getSearchBusy():
                if not self.remoteHits or self.remoteHits[0] != self.searchkeywords[mode]:
                    self.remoteHits = (self.searchkeywords[mode], [])
                numResults = len(self.remoteHits[1])
                ...
                for key,value in answers.iteritems():
                    ...
                    # Refilter xxx extra strict
                    if not 'xxx' in value['category'] and self.category.xxx_filter.isXXX(value['content_name']):
                        value['category'] = ['xxx']
                        
                    for cat in value['category']:
                        rank = self.category.getCategoryRank(cat)
                        if rank == -1:
                            if DEBUG:
...
#        starttime = time()
        ...
        for torrent in self.title_dict.get(beginTitle, []):
            if torrent['infohash'] != infohash and torrent['status'] == 'good' and self.category.hasActiveCategory(torrent):
        ...
        result = []
        for simt in simTorrents:
            torrent = self.info_dict.get(simt[2])
            if torrent:
