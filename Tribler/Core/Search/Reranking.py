...
class SwapFirstTwoReranker(Reranker):
    ...
    def rerank(self, hits, keywords, torrent_db, pref_db, mypref_db, search_db):
        ...
        self._logger.debug("reranking:  first torrent (%d): (num, score)= (%s, %s)" %\
            (torrent_id_0, num_hits_0, position_score_0))
        self._logger.debug("reranking: second torrent (%d): (num, score)= (%s, %s)" %\
            (torrent_id_1, num_hits_1, position_score_1))
