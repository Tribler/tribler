...
class SwapFirstTwoReranker(Reranker):
    ...
    def rerank(self, hits, keywords, torrent_db, pref_db, mypref_db, search_db):
        ...
        if not torrent_id_0 or not torrent_id_1:
            if DEBUG:
                print("reranking: torrent_id=0 in hits, exiting", file=sys.stderr)
        ...
        if DEBUG:
            print("reranking:  first torrent (%d): (num, score)= (%s, %s)" % (torrent_id_0, num_hits_0, position_score_0), file=sys.stderr)
            print("reranking: second torrent (%d): (num, score)= (%s, %s)" % (torrent_id_1, num_hits_1, position_score_1), file=sys.stderr)
        ...
        if (num_hits_0 < self.MAX_SEEN_BEFORE_RERANK or num_hits_1 < self.MAX_SEEN_BEFORE_RERANK):
            ...
            if DEBUG:
                print("reranking: not enough samples, not reranking", file=sys.stderr)
            return hits

        if (num_hits_0 / num_hits_1 > self.MAX_POPULAR_RATIO):
            # if number one is much more popular, keep everything as it is
            if DEBUG:
                print("reranking: first torrent is too popular, not reranking", file=sys.stderr)
            return hits
        ...
        if position_score_0 < position_score_1:
            if DEBUG:
                print("reranking: second torrent has better position score, reranking!", file=sys.stderr)
            h = hits[0]
            hits[0] = hits[1]
            hits[1] = h
        else:
            if DEBUG:
                print("reranking: second torrent does not have better position score, reranking!", file=sys.stderr)
