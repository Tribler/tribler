...
class Coordinator:
    ...
    def choose_helper(self, peerid):
        ...
        helper_challenge = self.get_challenge_from_peerid(peerid)
...
#            print >> sys.stderr, "coordinator: got_proxy_have: invalid payload in received PROXY_HAVE message. self.num_pieces=", self.num_pieces, "len(aggregated_string)=", len(aggregated_string)
        ...
        for d in self.downloader.downloads:
            ...
            if peer_challenge == self.get_challenge_from_peerid(peer_id):
    ...
    def get_challenge_from_peerid(self, peerid):
        """ Method used to retrieve a challenge from a peerid
        
        @param peerid: the peerid of the peer whose challenge is to be retrieved
        
        @return: a number: the challenge previously send to the peer, and stored in its peerid
        """
