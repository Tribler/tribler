...
from Tribler.Core.simpledefs import (DLSTATUS_DOWNLOADING, DLSTATUS_SEEDING, DLSTATUS_STOPPED_ON_ERROR, NTFY_ERROR,
                                     NTFY_FINISHED, NTFY_STARTED, NTFY_TORRENT, NTFY_TRIBLER, STATE_START_API_ENDPOINTS,
                                     STATE_START_CREDIT_MINING, STATE_START_LIBTORRENT, STATE_START_MAINLINE_DHT,
    ...
    def load_ipv8_overlays(self):
        ...
        if self.session.config.get_tunnel_community_enabled():
            ...
            if self.mainline_dht:
                ...
                dht_provider = MainlineDHTProvider(self.mainline_dht, self.session.config.get_ipv8_port())
            else:
