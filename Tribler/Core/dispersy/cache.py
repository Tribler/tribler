class CacheDict(object):
    ...
    every key by value V, where V is determined by the number of pokes performed since the last
    cleanup.

    Poke counters that reach zero or less are removed first, if we need to remove more items from
    memory the items with the least pokes are removed first.
