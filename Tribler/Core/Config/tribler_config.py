...
class TriblerConfig(object):
    ...
    def __init__(self, config=None):
        ...
        if config is None:
            ...
            if os.path.exists(file_name):
                config = ConfigObj(file_name, configspec=CONFIG_SPEC_PATH, encoding='utf-8')
            else:
                config = ConfigObj(configspec=CONFIG_SPEC_PATH, encoding='utf-8')
        self.config = config
        self.validate()
    ...
    def load(config_path=None):
        ...
        """
        return TriblerConfig(ConfigObj(config_path, configspec=CONFIG_SPEC_PATH, encoding='utf-8'))
    ...
    def copy(self):
        ...
        new_configobj = ConfigObj(self.config.copy(), configspec=self.config.configspec, encoding='utf-8')
