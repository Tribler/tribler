...
class TestTriblerTunnelCommunity(TestBase):  # pylint: disable=too-many-public-methods
    ...
    async def test_backup_exitnodes(self):
        ...
        assert len(self.nodes[0].overlay.get_candidates(PEER_FLAG_EXIT_BT)) >= 1
        # 2. Unload
    ...
    def test_readd_bittorrent_peers(self):
        ...
        assert mock_torrent not in self.nodes[0].overlay.bittorrent_peers
    ...
    def test_remove_circuit(self):
        ...
        assert 3 not in self.nodes[0].overlay.circuits
    ...
    def test_monitor_downloads_leave_swarm(self):
        ...
        self.nodes[0].overlay.download_states[b'a'] = 3
        self.nodes[0].overlay.monitor_downloads([])
        assert b"a" not in self.nodes[0].overlay.swarms
    ...
    def test_monitor_downloads_intro(self):
        ...
        self.nodes[0].overlay.download_states[b'a'] = 3
        self.nodes[0].overlay.monitor_downloads([])
        assert mocked_remove_circuit.circuit_id == 0
    ...
    def test_update_torrent(self):
        ...
        assert mock_download in self.nodes[0].overlay.bittorrent_peers
    ...
    async def test_payouts(self):
        ...
        assert self.nodes[0].overlay.tunnels_ready(2) == 1.0
        ...
        assert self.nodes[0].overlay.bandwidth_community.database.get_my_balance() < 0
        assert self.nodes[1].overlay.bandwidth_community.database.get_my_balance() > 0
        assert self.nodes[2].overlay.bandwidth_community.database.get_my_balance() > 0
        ...
        balances.sort()
        assert balances[0] == -750 * 1024 * 1024
        assert balances[1] == 250 * 1024 * 1024
        assert balances[2] == 500 * 1024 * 1024
    ...
    async def test_circuit_reject_too_many(self):
        ...
        assert self.nodes[0].overlay.tunnels_ready(1) == 0.0
    ...
    async def test_payouts_e2e(self):
        ...
        for circuit_id in removed_circuits:
            self.nodes[0].overlay.remove_circuit(circuit_id, destroy=1)
        for i in range(3):
            assert self.nodes[i].overlay.bandwidth_community.database.get_my_balance() == balances[i]
    ...
    async def test_win_competing_slot(self):
        ...
        # Assert whether we didn't create the circuit
        assert self.nodes[0].overlay.tunnels_ready(1) == 1.0
    ...
    async def test_payout_on_competition_kick(self):
        ...
        next(iter(self.nodes[0].overlay.circuits.values())).bytes_down = 250 * 1024 * 1024

        assert self.nodes[0].overlay.tunnels_ready(1) == 1.0
        assert self.nodes[2].overlay.exit_sockets

        self.nodes[1].overlay.build_tunnels(1)
        await self.deliver_messages()
        assert self.nodes[2].overlay.exit_sockets
        assert self.nodes[1].overlay.tunnels_ready(1) == 1.0

        # Check whether the exit node has been paid
        assert self.nodes[2].overlay.bandwidth_community.database.get_my_balance() >= 250 * 1024 * 1024
    ...
    async def test_intro_point_slot(self):
        ...
        exit_socket = next(iter(self.nodes[1].overlay.exit_sockets.values()))
        assert exit_socket.circuit_id in self.nodes[1].overlay.random_slots

        self.nodes[0].overlay.send_cell(circuit.peer,
                                        EstablishIntroPayload(circuit.circuit_id, int(random() * 2 ** 16), b'', b''))
        await self.deliver_messages()
        assert exit_socket.circuit_id not in self.nodes[1].overlay.random_slots
