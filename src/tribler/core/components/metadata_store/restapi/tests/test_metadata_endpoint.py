...
from tribler.core.components.metadata_store.restapi.metadata_endpoint import TORRENT_CHECK_TIMEOUT, MetadataEndpoint
...
async def test_update_multiple_metadata_entries(metadata_store, add_fake_torrents_channels, rest_api):
    ...
    with db_session:
        ...
        assert entry1.title == NEW_NAME1
        assert entry1.status == UPDATED

        entry2 = metadata_store.ChannelNode.get(rowid=md2.rowid)
        assert entry2.title == NEW_NAME2
