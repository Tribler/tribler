...
export function Autocomplete({
    ...
    placeholder: string;
    completions: (filter: string) => Promise<string[]>;
                    ...
                    <div className="flex flex-wrap gap-1">
                        <input
                            ...
                            onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                    const query =
                                        selectedSuggestion > 0 ? suggestions[selectedSuggestion - 1] : inputValue;
                        ...
                        <div className="max-h-[300px] overflow-y-auto overflow-x-hidden absolute top-0 z-10 w-full rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in">
                            {suggestions.length > 0 &&
                                suggestions.map((suggestion, index) => (
                                    <div
                                        className={`p-1 text-foreground h-full overflow-auto  ${selectedSuggestion === index + 1 ? "bg-accent" : ""}`}
                                        key={index + "a"}>
                                        <div
                                            className="relative flex select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none cursor-pointer"
