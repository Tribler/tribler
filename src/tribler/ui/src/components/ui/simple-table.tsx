            ...
            <PopoverContent
                ...
                <div className="grid gap-4">
                    <div className="space-y-2">
                        ...
                        <p className="text-sm text-muted-foreground">{t("ColumnSettingsDescription")}</p>
                    </div>
    ...
    return ({column}) => {
        return (
            <ColumnSetup
                ...
                addFilter={addFilter}></ColumnSetup>
        );
    };
}
...
function SimpleTable<T extends object>({
    ...
    useEffect(() => {
        if (onSelectedRowsChange) onSelectedRowsChange(table.getSelectedRowModel().flatRows.map((row) => row.original));
    ...
    return (
        <>
            <ScrollArea className={cn("relative w-full overflow-auto", className)} ref={parentRef} style={style}>
                <Table>
                        ...
                        {table.getHeaderGroups().map((headerGroup) => (
                            <TableRow
                                ...
                                {allowColumnToggle && (
                                    <TableHead
                                        key="toggleColumns"
                                        className="w-2 pl-1 pr-3 cursor-pointer hover:text-black dark:hover:text-white">
                                        <DropdownMenu>
                                                        ...
                                                        <DropdownMenuItem key={`toggleColumns-${column.id}`}>
                                                            <label
                                                                ...
                                                                {flexRender(column.columnDef.header, {
                                                                    table,
                                                                    column: fakeColumn,
                                                                    header: {column: fakeColumn} as Header<
                                                                        any,
                                                                        unknown
                                                                    >,
                                                                })}
                            ...
                            items.map((item, rowIndex) => {
                                ...
                                return (
                                    <TableRow
                                        ...
                                        onClick={(event) => {
                                            if (!allowSelect && !allowMultiSelect) return;
                                            ...
                                            if (startRow && allowMultiSelect && event.shiftKey) {
                                                ...
                                                for (
                                                    let i = Math.min(startIndex, stopIndex);
                                                    i <= Math.max(startIndex, stopIndex);
                                                    i++
                                                ) {
                                                    selection[rows[i].id] = true;
                                                }
