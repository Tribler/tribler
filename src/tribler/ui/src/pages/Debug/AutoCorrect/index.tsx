    ...
    useInterval(
        async () => {
            ...
            if (!(response === undefined) && !isErrorDict(response)) {
                ...
                setVocabularyContinuations(
                    response.filter((word) => !word.startsWith("▁") && !internal.includes(word)).sort()
                );
            }
        },
        5000,
        ...
        <div className="w-full h-full p-2">
            ...
            {vocabularyPrefixes.map((elem, index) => {
                return (
                    <span
                        key={index}
                        className="inline-flex items-center m-1 rounded-md bg-card px-2 py-1 text-xs font-medium text-card-foreground inset-ring inset-ring-gray-500/10">
                        {elem.substr(1)}
                    </span>
                );
            })}
            ...
            {vocabularyContinuations.map((elem, index) => {
                return (
                    <span
                        key={index}
                        className="italic inline-flex items-center m-1 rounded-md bg-linear-to-r from-card to-90% to-accent  px-2 py-1 text-xs font-medium text-card-foreground inset-ring inset-ring-gray-500/10">
                        {elem}
                    </span>
                );
            })}
