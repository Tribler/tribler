            ...
            return (
                <span
                    className={`font-medium ${
                        row.original.peers.length < 20
                            ? `text-green-400`
                            : row.original.peers.length < row.original.max_peers
                              ? "text-yellow-400"
                              : "text-red-400"
                    }`}>
...
export default function Overlays() {
    ...
    return (
        <ResizablePanelGroup direction="vertical">
            ...
            <ResizableHandle
                className={`border-2 border-gray-300 dark:border-gray-600 ${selectedOverlay ? "flex" : "hidden"}`}
            />
            <ResizablePanel defaultSize={50} className={`${selectedOverlay ? "flex" : "hidden"}`}>
                <SimpleTable data={selectedOverlay?.peers || []} columns={peerColumns} />
            </ResizablePanel>
        </ResizablePanelGroup>
    );
}
