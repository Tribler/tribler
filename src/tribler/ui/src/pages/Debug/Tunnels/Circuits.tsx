...
const circuitColumns: ColumnDef<Circuit>[] = [
    ...
    },
    {
        ...
        cell: ({row}) => {
            return (
                <span>
                    {row.original.actual_hops} / {row.original.goal_hops}
                </span>
            );
        },
    },
...
export default function Circuits() {
    ...
    return (
        <ResizablePanelGroup direction="vertical">
            ...
            <ResizableHandle
                className={`border-2 border-gray-300 dark:border-gray-600 ${selectedCircuit ? "flex" : "hidden"}`}
            />
            <ResizablePanel defaultSize={50} className={`${selectedCircuit ? "flex" : "hidden"}`}>
                <ResponsiveContainer width="95%" height="95%">
                    <LineChart data={speeds} margin={{top: 20, right: 10, left: 20, bottom: 5}}>
                        <CartesianGrid
                            strokeDasharray="3 3"
                        ...
                        <Line
                            ...
                            stroke={`${window.document.documentElement.classList.contains("dark") ? "#38bdf8" : "#0369a1"}`}
                        />
                    </LineChart>
                </ResponsiveContainer>
