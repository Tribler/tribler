...
export default function Health() {
    ...
    const drawBoop = (ctx: CanvasRenderingContext2D, x: number, xscale: number, yscale: number) => {
        ...
        drawLine(
            ctx,
            "#84D684",
            x + Math.round(5 * xscale),
            midy - Math.round(10 * yscale),
            x + Math.round(10 * xscale),
            midy
        );
        ...
        drawLine(
            ctx,
            "#84D684",
            x + Math.round(15 * xscale),
            midy,
            x + Math.round(20 * xscale),
            midy + Math.round(10 * yscale)
        );
        drawLine(
            ctx,
            ...
            x + Math.round(20 * xscale),
            midy + Math.round(10 * yscale),
            x + Math.round(30 * xscale),
            midy - Math.round(50 * yscale)
        );
        drawLine(
            ctx,
            "#84D684",
            x + Math.round(30 * xscale),
            midy - Math.round(50 * yscale),
            x + Math.round(50 * xscale),
            midy + Math.round(40 * yscale)
        );
        drawLine(
            ctx,
            "#84D684",
    ...
    const target =
        drifts.length >= 2
            ? (
                  drifts[drifts.length - 1].timestamp -
                  drifts[drifts.length - 2].timestamp -
                  drifts[drifts.length - 1].drift
              ).toFixed(2)
            : "N/A";
