        ...
        <div className="p-6">
            <div className="grid grid-cols-4 gap-2 items-center">
                ...
                <Suspense fallback={<Label key="current_version_label">...</Label>}>
                    <Label key="current_version_label">{version ? version : "..."}</Label>
                </Suspense>
                <Suspense fallback={<Label key="current_version_available"></Label>}>
                    {newVersion ? (
                        <Label key="current_version_available">
                            {t("VersionAvailable")}: {newVersion}
                        </Label>
                    ) : (
                ...
                <Label key="spacer5"></Label>

                <Label className="whitespace-nowrap pr-5 font-bold" key="old_version_header">
                    {t("VersionOld")}
                </Label>
                <Label key="spacer6"></Label>
                <Label key="spacer7"></Label>
                <Label key="spacer8"></Label>

                {versions
                    .reduce((r: string[], e: string) => {
                        r.push(e, e, e, e);
                        return r;
                    }, new Array<string>())
                    .map(function (old_version: string, i: number) {
                        switch (i % 4) {
                            case 0: {
                            ...
                            case 2: {
                                return typeof canUpgrade === "string" && canUpgrade == old_version ? (
                                    isUpgrading ? (
                                        <div className="flex justify-center p-5 gap-1">
                                            <RefreshCw
                                                opacity="0.5"
                                                className="animate-spin duration-500"
                                                key={`import_status_${i}`}
                                            />
                                            <Label className="content-center text-muted-foreground">
                                                {t("VersionUpgrading")}...
                                            </Label>
                                        </div>
                                    ) : (
                                        <Button
                                            variant="default"
                                            type="submit"
                                            onClick={(e) => clickedImport(e, old_version)}
                                            key={`import_status_${i}`}>
                                            {t("VersionImport")}
                                        </Button>
                                    )
                            ...
                            default: {
                                return (
                                    <Button
                                        variant="destructive"
                                        type="submit"
                                        onClick={(e) => clickedRemove(e, old_version)}
                                        key={`remove_button_${i}`}>
                                        {t("VersionRemove")}
                                    </Button>
                                );
                            }
