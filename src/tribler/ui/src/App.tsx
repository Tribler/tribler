...
function reportError() {
    ...
    if (error_popup_text) {
        const err_text =
            "Hi! I was using Tribler " +
            (triblerService.version || "uninitialized") +
...
function searchError() {
    ...
    if (error_popup_text?.textContent) {
        ...
        if (err_lines.length > 1) {
            ...
            const err_file = err_file_raw
                .substring(last_f_name + 1)
                .replace(/"/g, "")
                .replace(/,/g, "");
    ...
    return (
        <>
            <div
                id="error_popup"
                role="alert"
                className="overflow-hidden mx-8 mt-2 h-10 hover:h-96 hidden transition-height duration-500 ease-in-out">
                <div className="bg-red-500 text-white font-bold rounded-t px-4 py-2">{t("ErrorNotification")}</div>
                <div
                    className="border border-t-0 border-red-400 rounded-b bg-card px-4 py-3 text-red-600"
                    style={{whiteSpace: "pre"}}>
                    <p id="error_popup_text" className="overflow-y-scroll max-h-64"></p>
                    <div className="w-1/2 justify-start">
                        <Button
                            className="h-10 pl-2 my-2 w-1/3 mx-2 justify-start rounded-none text-muted-foreground bg-secondary"
                            variant="outline"
                            onClick={searchError}>
                            {t("ErrorSearchButton")}
                        </Button>
