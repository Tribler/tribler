...
class SubscribedChannelsPage(QWidget):
    ...
    def received_subscribed_channels(self, results):
        ...
        if len(results['subscribed']) == 0:
            self.window().subscribed_channels_list.set_data_items([(LoadingListItem, "You are not subscribed to any channel.")])
            return
    ...
    def on_subscription_added(self, action):
        if action == 0:
            ...
            self.request_mgr.perform_request("channels/subscribed/%s" % self.dialog.dialog_widget.dialog_input.text(), self.on_channel_subscribed, method='PUT')
