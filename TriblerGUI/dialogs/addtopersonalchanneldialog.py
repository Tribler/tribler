...
class AddToChannelDialog(DialogContainer):
    ...
    def __init__(self, parent):
        ...
        self.channels_tree = {}
        root_chan = QtWidgets.QTreeWidgetItem(self.dialog_widget.channels_tree_wt, ["My channel"])
        self.id2wt_mapping = {0: root_chan}
        self.wt2id_mapping = {root_chan: 0}
    ...
    def on_create_new_channel_clicked(self):
        ...
        channel_id = self.wt2id_mapping[selected[0]] if selected else 0
    ...
    def on_item_expanded(self, item):
        expanded_id = self.wt2id_mapping[item]
        # Load the grand-children
        for channel_id in self.channels_tree.get(expanded_id, None):
    ...
    def get_selected_channel_id(self):
        selected = self.dialog_widget.channels_tree_wt.selectedItems()
        return None if not selected else self.wt2id_mapping[selected[0]]
    ...
    def on_channel_contents(self, response, channel_id):
        ...
        for subchannel in response["results"]:
            ...
            if subchannel_id in self.id2wt_mapping:
                continue
            wt = QtWidgets.QTreeWidgetItem(self.id2wt_mapping[channel_id], [subchannel["name"]])
            self.id2wt_mapping[subchannel_id] = wt
            self.wt2id_mapping[wt] = subchannel_id
