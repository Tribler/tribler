    ...
    def gui_states_callback(self,dslist):
        ...
        try:
            ...
            for ds in dslist:
                ...
                #if DEBUG:
                print >>sys.stderr,"%s %s %.1f%% dl %.1f ul %.1f n %d\n" % (safename, dlstatus_strings[ds.get_status()],100.0*ds.get_progress(),ds.get_current_speed(DOWNLOAD),ds.get_current_speed(UPLOAD),ds.get_num_peers())
    ...
    try:
        ...
        if not ALLOW_MULTIPLE and single_instance_checker.IsAnotherRunning():
            ...
            try:
                ...
                iss = hp.heap()
                print >>sys.stderr,"HELP0",iss
                print >>sys.stderr,"HELP1",iss.more
                iss = iss.more
                print >>sys.stderr,"HELP2",iss.more
                iss = iss.more
                print >>sys.stderr,"HELP3",iss.more
                iss = iss.more
                print >>sys.stderr,"HELP4",iss.more
                iss = iss.more
                print >>sys.stderr,"HELP5",iss.more
                
                
                byrcs = h.byrcs
                lenrcs = len(byrcs)
                print >>sys.stderr,"Inspect HEAP len",lenrcs
                for i in range(0,lenrcs):
                    elem = byrcs[i]
                    elemstr = `elem`
                    if elemstr.endswith("100 list"):
                        print >>sys.stderr,"ELEM",elem
                        first = elem.byid
                        while True:
                            print >>sys.stderr,"CONT",first
                            first = first.more
                            if first.startindex > len(first.top):
                                break
            except:
                print_exc()
