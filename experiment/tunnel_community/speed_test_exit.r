...
speed_test_exit <- read.table("speed_test_exit.txt", header=T, quote="\"")
...
p <- ggplot(upload_speeds, aes(x=Time, y=Speed, colour=factor(Circuit), group=Circuit)) +
    ...
    ggtitle("Upload speed for exit circuits")
p
ggsave("speed_test_exit_upload.png", width=10, height=6, dpi=100)
...
p <- ggplot(download_speeds, aes(x=Time, y=Speed, colour=factor(Circuit), group=Circuit)) +
	...
	stat_summary(aes(colour="mean", group=1), fun=mean, geom="line", size=1.1) +
	scale_y_continuous(name="Speed (KiB/s)") +
    ggtitle("Download speed for exit circuits")
p
ggsave("speed_test_exit_download.png", width=10, height=6, dpi=100)
q(save="no")
